...
<template>
  <nav v-if="totalPages > 1" aria-label="Paginación" class="flex items-center justify-center mt-8 mb-4">
    <ul class="flex items-center -space-x-px h-10 text-base">
      ...
      <li>
        <button
          ...
          :class="{ 'cursor-not-allowed opacity-50': isFirstPage }"
        >
      ...
      <li v-for="page in pageNumbers" :key="page">
        <button
          ...
          :class="[
            ...
            currentPage === page ? 'text-blue-600 bg-blue-50 border-blue-300 hover:bg-blue-100 hover:text-blue-700 z-10' : 'text-gray-500 bg-white'
          ]"
        >
          {{ page }}
        </button>
        <span v-else class="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300">
          {{ page }} <!-- Para el '...' si lo implementas -->
        </span>
      </li>
